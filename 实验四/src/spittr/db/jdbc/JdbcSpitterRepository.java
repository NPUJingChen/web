...
public class JdbcSpitterRepository implements SpitterRepository {
	...
	@Override
	public PaginationSupport<Spitter> findPage(int pageNo,int pageSize){
		int totalCount = (int) count();
		int startIndex = PaginationSupport.convertFromPageToStartIndex(pageNo, pageSize);
		if (totalCount < 1)
			return new PaginationSupport<Spitter>(new ArrayList<Spitter>(0), 0);

		List<Spitter> items = jdbc.query(SELECT_PAGE_SPITTER, new JdbcSpitterRepository.SpitterRowMapper(), pageSize, startIndex);
		PaginationSupport<Spitter> ps = new PaginationSupport<Spitter>(items, totalCount, pageSize, startIndex);
		return ps;
	}
	...
	private static final String SELECT_PAGE_SPITTER = SELECT_SPITTER + " order by id  limit ? offset  ?";
